...
├── 00_INDEX.md                    ← 本文件
│
├── Projects/                      ← 🔥 Nuvoton 官方範例專案
│   ├── HSUSBD_Mass_Storage_ShortPacket/   ← MSC Short Packet 參考
│   ├── HSUSBD_VENDOR_LBK/                 ← VENDOR_LBK 參考
│   ├── Library/                           ← 驅動程式庫
│   │   ├── CMSIS/
│   │   ├── Device/
│   │   ├── StdDriver/
...
│   │   ├── USBD_Mass_Storage_Flash/       ← MSC + Flash
│   │   ├── USBD_VENDOR_LBK/               ← VENDOR_LBK
│   │   ├── USBD_HID_Transfer/             ← HID Transfer
│   │   └── ...（共 57+ 個範例）
...
└── Reference/                    ← 🔧 參考工具與資料
    ├── flash_backup/
    │   ├── m487_flash.bin        ← M487 Flash 完整備份
...
| 範例資料夾 | 用途 | 對應本專案 |
|-----------|------|-----------|
| `HSUSBD_VENDOR_LBK/` | USB VENDOR Command (LBK) | MSC Debug Channel (T025) |
| `USBD_Mass_Storage_Flash/` | MSC + SPI Flash | USB MSC RAM Disk |
| `USBD_HID_Transfer/` | USB HID Transfer | HID I2C Bridge (T001) |
| `USBD_HID_Transfer_And_MSC/` | HID + MSC 複合 | 參考 `firmware/composite/` |
| `Library/StdDriver/src/usbd.c` | USB Device Driver 參考 | USB 初始化 |
| `Library/StdDriver/src/scu.c` | System Control | Clock/PLL 設定 |
...
| 單元測試 | `Reference/UnitTest/` | Ceedling / GoogleTest / Unity 框架 |
...
| 需求 | 前往 |
|------|------|
| **MSC + VENDOR_LBK 參考** | `Projects/HSUSBD_VENDOR_LBK/` |
| **HID Transfer 參考** | `Projects/USB_HS_Samples/USBD_HID_Transfer/` |
| **StdDriver USB** | `Projects/Library/StdDriver/src/usbd.c` |
